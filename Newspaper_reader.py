...
try:
    ...
    a = response.json()
    p = a['articles']
    print("Performing Recognitions for Compatibilty...")
    speak("Performing Recognitions for Compatibilty...")
    time.sleep(2)
    speak("Your Operating System is , {platform.platform()}. And your Hardware is ,{platform.machine()}")
    print("Your OS: {platform.platform()} \nHardware:{platform.machine()}")
    time.sleep(2)
...
if input=="" \
    ...
    for i in range(len(p)):
        print("News number {i + 1}")
        speak("News number {i + 1} about {p[i]['description']}")
        time.sleep(1)
        speak(p[i]['content'])
        time.sleep(1)
        speak("For more information kindly visit {p[i]['url'].split('//')[1][:40]}")
        print("For more information kindly visit {p[i]['url'][:40]}")
        print("\n")
        time.sleep(2)
